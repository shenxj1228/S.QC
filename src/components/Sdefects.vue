...
export default {
  ...
  mounted: function() {
    ...
    EventBus.$on("fullscreen", isFullScreen => {
      setTimeout(() => {
        this.table.height =
          this.$refs.layout.$el.offsetHeight - this.$refs.page.$el.offsetHeight;
        this.table.width =
    ...
    buildTableColumns() {
      if (this.resultFields.length === 0) {
        this.$Modal.warning({
          ...
          onOk: () => {
            this.$router.push({
              path: `/setting/${this.domainName}@${this.projectName}`
            });
          }
        });
        return;
      ...
      api
        ...
        .then(fieldObjs => {
          this.columns = this.resultFields.map((currentValue, index, array) => {
    ...
    queryDefect() {
      ...
      Object.keys(this.formQuery).forEach((val, index) => {
        if (Array.isArray(this.formQuery[val])) {
          if (this.formQuery[val][0] != "" && this.formQuery[val][1] != "") {
            ...
            let endDate = this.formQuery[val][1]
              ...
              .replace(/\//g, "-");
            queryStr =
              queryStr + `${val}[>='${startDate}' And <='${endDate}'];`;
          }
        } else {
